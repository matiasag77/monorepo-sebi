...
export default function WebRunSebi({
  email,
  userId,
  apiBaseUrl = "",
}: WebRunSebiProps) {
  ...
  const handleSend = async (messageText?: string) => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/api/v1/chat_sebi`, {
        method: "POST",
      ...
      const data: SebiApiResponse = await res.json()
      const receivedAt = new Date().toISOString()

      const botMessage: Message = {
        text: data.answer || "Procesamiento completado.",
        sender: "bot",
        tables: data.tables || [],
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 relative overflow-hidden">
        {messages.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center h-full px-4 py-8 animate-fade-in">
